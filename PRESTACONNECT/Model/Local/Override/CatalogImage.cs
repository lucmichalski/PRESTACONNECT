...
namespace PRESTACONNECT.Model.Local
{
    public partial class CatalogImage
    {
        ...
        public string GetExtension
        {
            get
            {
                return Path.GetExtension(ImaCat_Image);
            }
        }
            ...
            get
            {
                ...
                switch (Core.Global.GetConfig().ConfigLocalStorageMode)
                {
                    ...
                    case Core.Parametres.LocalStorageMode.simple_system:
                        file = System.IO.Path.Combine(Global.GetConfig().Folders.SmallCatalog, ImaCat_Image);
                        break;
                    //case Core.Parametres.LocalStorageMode.advanced_system:
                    //    file = System.IO.Path.Combine(advanced_folder, String.Format("{0}_small" + GetExtension, ImaCat_Id));
                    //    break;
                }
                return file;
            }
        ...
        public string TempFileName
        {
            get
            {
                ...
                switch (Core.Global.GetConfig().ConfigLocalStorageMode)
                {
                    default:
                    case Core.Parametres.LocalStorageMode.simple_system:
                        file = System.IO.Path.Combine(Global.GetConfig().Folders.TempCatalog, ImaCat_Image);
                        break;
                    //case Core.Parametres.LocalStorageMode.advanced_system:
                    //    file = System.IO.Path.Combine(advanced_folder, String.Format("{0}" + GetExtension, ImaCat_Id));
                    //    break;
                }
                return file;
            }
        ...
        public string FileName(string PsImageType)
        {
            ...
            switch (Core.Global.GetConfig().ConfigLocalStorageMode)
            {
                default:
                case Core.Parametres.LocalStorageMode.simple_system:
                    file = System.IO.Path.Combine(Core.Global.GetConfig().Folders.RootArticle, String.Format("{0}-{1}" + GetExtension, ImaCat_Id, PsImageType));
                    break;
                //case Core.Parametres.LocalStorageMode.advanced_system:
                //    file = System.IO.Path.Combine(advanced_folder, String.Format("{0}-{1}" + GetExtension, ImaCat_Id, PsImageType));
                //    break;
            }
            return file;
        }
        ...
        public void EraseFiles()
        {
            ...
            foreach (var item in new Model.Prestashop.PsImageTypeRepository().ListCategorie(1))
            {
                try
                {
